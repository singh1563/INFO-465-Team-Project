...
function setupCourseSearchPage() {
  ...
  function renderResults(sections) {
    ...
    if (sections.length === 0) {
      resultsContainer.innerHTML = `
        <article class="feature-card full-width-card">
          ...
          <p>Try searching by department code, instructor name, or course number.</p>
        </article>
      `;
    ...
    sections.forEach((section) => {
      ...
      const availabilityClass = isSectionFull(section) ? "badge badge-closed" : "badge badge-open";
      const availabilityText = isSectionFull(section) ? "Full / Closed" : "Open";
      ...
      article.innerHTML = `
        ...
        <p><strong>Meeting Time:</strong> ${section.days}, ${section.time}</p>
        <p><strong>Enrollment:</strong> ${section.enrolled} / ${section.capacity}</p>
        <p><strong>Open Seats:</strong> ${openSeats}</p>
        <p><span class="${availabilityClass}">${availabilityText}</span></p>
        <a href="registration.html">View Registration Options</a>
      `;
      resultsContainer.appendChild(article);
    });

    resultSummary.textContent = `${sections.length} matching course section(s) found.`;
  }

  function filterSections() {
    const departmentValue = departmentInput.value.trim().toLowerCase();
    const instructorValue = instructorInput.value.trim().toLowerCase();
    const courseNumberValue = courseNumberInput.value.trim().toLowerCase();

    const isQuickSearch = quickSearchValue &&
    ...
    if (isQuickSearch) {
      ...
      filteredSections = courseSections.filter((section) => {
        return section.department.toLowerCase().includes(value) ||
      ...
      filteredSections = courseSections.filter((section) => {
        const matchesDepartment = departmentValue === "" || section.department.toLowerCase().includes(departmentValue);
        const matchesInstructor = instructorValue === "" || section.instructor.toLowerCase().includes(instructorValue);
        const matchesCourseNumber = courseNumberValue === "" || section.courseNumber.toLowerCase().includes(courseNumberValue) || section.courseCode.toLowerCase().includes(courseNumberValue);
        return matchesDepartment && matchesInstructor && matchesCourseNumber;
      });
    }
...
function setupStudentRegistrationPage() {
  ...
  function renderLoggedOutState() {
    studentSummary.innerHTML = `
      ...
      <p>Use the demo credentials to access the student schedule.</p>
      <p class="demo-credentials"><strong>Demo Student ID:</strong> V00999999<br><strong>Password:</strong> student123</p>
    `;
    enrolledList.innerHTML = `
      <div>
        <h3>Login Required</h3>
        <p>A student must log in before enrolled classes can be displayed.</p>
      </div>
    `;
  }

  function renderStudentDashboard(student) {
    const enrolledSections = student.enrolledSectionIds.map(findSectionById).filter(Boolean);
    const totalCredits = enrolledSections.reduce((sum, section) => sum + section.credits, 0);
    ...
    enrolledSections.forEach((section) => {
      ...
      card.innerHTML = `
        ...
        <p><strong>Section:</strong> ${section.section}</p>
        <p><strong>Instructor:</strong> ${section.instructor}</p>
        <p><strong>Time:</strong> ${section.days}, ${section.time}</p>
        <p><strong>Modality:</strong> ${section.modality}</p>
      `;
    ...
    rulesCard.innerHTML = `
      ...
      <p>Duplicate registrations and full course sections are blocked by validation.</p>
    `;
    enrolledList.appendChild(rulesCard);
  }

  function renderRegistrationOptions() {
    registrationOptions.innerHTML = "";

    courseSections.slice(0, 4).forEach((section) => {
      const openSeats = getOpenSeats(section);
      ...
      article.innerHTML = `
        ...
        <p><strong>Validation:</strong> ${isFull ? "Enrollment limit reached." : "Available for registration review."}</p>
        <button type="button" ${isFull ? "disabled" : ""} onclick="alert('${isFull ? "Registration blocked: this course section is full." : "Registration review opened for this course section."}')">${isFull ? "Section Full" : "Review Registration"}</button>
      `;
  ...
  loginForm.addEventListener("submit", function (event) {
    ...
    const student = studentAccounts[studentId];

    if (!student || student.password !== password) {
      showMessage(loginMessage, "Login failed. Use Student ID V00999999 and password student123 for the demo.", "error");
      renderLoggedOutState();
      return;
    }

    localStorage.setItem("activeStudentId", studentId);
    showMessage(loginMessage, "Student login successful. Enrolled classes are now displayed.", "success");
    renderStudentDashboard(student);
  });

  const activeStudentId = localStorage.getItem("activeStudentId");
  if (activeStudentId && studentAccounts[activeStudentId]) {
    renderStudentDashboard(studentAccounts[activeStudentId]);
  } else {
    renderLoggedOutState();
  }
